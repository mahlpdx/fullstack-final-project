      ...
      <div className="flex flex-row items-center gap-4 w-full h-full">
            ...
            {tracks.map((track) => (
              <>
                <button
                  ...
                  className="  font-geomatik text-zinc-200 mb-3 rounded-2xl bg-cover flex flex-row "
                  style={{
                ...
                >
                  <div
                    className={
                      selected === track.id
                        ? " border-4 border-indigo-600 backdrop-blur-sm bg-zinc-900/80 rounded-2xl w-full p-4 h-full flex flex-row gap-3"
                        : " backdrop-blur-sm bg-zinc-900/80 rounded-2xl w-full p-4 h-full flex flex-row gap-3"
                    }
                  >
                    <img
        ...
        <div className={!show ? "hidden" : "flex flex-col w-full h-full"}>
          <div className="w-full flex flex-col items-center ">
            <div className="flex flex-row items-center bg-zinc-900/80 px-3 font-geomatik py-2  rounded-full gap-3">
              <button
                ...
                className={
                  !m1
                    ? " text-sm px-3 py-1 text-zinc-400 rounded-3xl border-2 border-zinc-400 hover:text-zinc-100 hover:bg-zinc-400  transition duration-300 ease-in-out"
                    : "text-sm px-3 py-1 bg-pink-600 text-pink-200 rounded-3xl border-2 border-pink-600"
                }
              >
                Relative Popularity
              </button>
          ...
          <div className="h-[29rem] w-full mt-3 h-full  bg-zinc-900/50 rounded-3xl p-6 flex flex-col items-start">
            <div
              className={
                !m1 ? "hidden" : "flex h-full flex-col items-start w-full"
              }
            >
              <div className="text-lg font-geomatik text-zinc-200/70 mb-3 ml-4">
                Relative Popularity of Top Tracks
              </div>

              <StockChart info={datac} className="w-full h-full" />
            </div>
            <div
              className={!m2 ? "hidden" : "flex flex-col items-start w-full"}
            >
              <div className="text-lg font-geomatik text-zinc-200/70 mb-3 ml-4">
                {tc.name} Audio features
              </div>
            ...
            <div
              className={
                !m3 ? "hidden" : "flex h-full flex-col items-start w-full"
              }
            >
              <div className="text-lg font-geomatik text-zinc-200/70 mb-3 ml-4">
                {tc.name} Feature Graph
              </div>
